...
export async function generateDemoData(): Promise<Partial<FormData>> {
  ...
  if (archetype === 'weight-loss') {
    ...
    goalLevelWeightLoss = pickRandom(['health-minimum', 'average', 'above-average', 'elite'], rng);
  } else if (archetype === 'muscle-gain') {
    clientGoals = pickRandom([['build-muscle'], ['build-muscle', 'build-strength']], rng);
    goalLevelMuscle = pickRandom(['health-minimum', 'average', 'above-average', 'elite'], rng);
    if (clientGoals.includes('build-strength')) {
      goalLevelStrength = pickRandom(['health-minimum', 'average', 'above-average', 'elite'], rng);
    }
  } else if (archetype === 'fitness-improvement') {
    ...
    clientGoals = pickRandom([['general-health'], ['weight-loss', 'general-health'], ['improve-fitness', 'general-health']], rng);
