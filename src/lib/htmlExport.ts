...
import { calculateAllHtmlExportData } from './utils/htmlExportCalculations';
...
export async function generateInteractiveHtml(params: {
  ...
  // Pre-calculate all derived values using extracted utilities
  const { goals, focusAreas, strengths, priorityFocus, lifestyleRecs, nutritionAdvice } =
    calculateAllHtmlExportData(formData, scores);
      ...
      return (
        <div className="container">
          ...
          <div style={{ marginTop: '32px', paddingTop: '24px', borderTop: '1px solid #e2e8f0', textAlign: 'center', fontSize: '12px', color: '#94a3b8' }}>
            Generated by FitnessFlow Assessment Engine
          </div>
        </div>
      );
    }
