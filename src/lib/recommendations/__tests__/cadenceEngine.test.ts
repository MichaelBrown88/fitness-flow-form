...
describe('generateCadenceRecommendations — base intervals', () => {
  it('returns clinical baselines when scores are neutral and no goals', () => {
    ...
    expect(result.schedule.bodycomp.intervalDays).toBe(BASE_CADENCE_INTERVALS.bodycomp);
  ...
  it('returns medium priority by default', () => {
    ...
    expect(result.schedule.bodycomp.priority).toBe('medium');
...
describe('generateCadenceRecommendations — score-based modifiers', () => {
  it('reduces interval by 50% for critical scores (< 40)', () => {
    ...
    expect(result.schedule.bodycomp.intervalDays).toBe(
      Math.round(BASE_CADENCE_INTERVALS.bodycomp * 0.5)
    );
    expect(result.schedule.bodycomp.priority).toBe('high');
  });
...
describe('generateCadenceRecommendations — visceral fat override', () => {
  it('sets bodycomp to 14 days when visceral fat > 10', () => {
    const form = mockFormData({ visceralFatLevel: '15' });
    const result = generateCadenceRecommendations(form, mockScores());

    expect(result.schedule.bodycomp.intervalDays).toBe(14);
    expect(result.schedule.bodycomp.priority).toBe('high');
    expect(result.schedule.bodycomp.reason).toContain('visceral fat');
  });
  ...
  it('adds a critical_finding warning for elevated visceral fat', () => {
    ...
    const warning = result.warnings.find(
      w => w.pillar === 'bodycomp' && w.type === 'critical_finding'
    );
    expect(warning).toBeDefined();
    expect(warning?.message).toContain('12');
  });

  it('does not override bodycomp when visceral fat <= 10', () => {
...
describe('generateCadenceRecommendations — goal-based modifiers', () => {
  it('sets bodycomp to 14 days for weight-loss goal with hydration warning', () => {
  ...
  it('sets bodycomp to 21 days for build-muscle goal', () => {
    const form = mockFormData({ clientGoals: ['build-muscle'] });
    const result = generateCadenceRecommendations(form, mockScores());

    expect(result.schedule.bodycomp.intervalDays).toBe(21);
  });
});
...
describe('getEffectiveInterval', () => {
  const recommended: PillarCadence = {
    bodycomp: { intervalDays: 14, priority: 'high', reason: 'test' },
  ...
  it('returns recommended interval when no custom override', () => {
    expect(getEffectiveInterval(recommended, undefined, 'bodycomp')).toBe(14);
    expect(getEffectiveInterval(recommended, {}, 'bodycomp')).toBe(14);
  });

  it('returns custom interval when override exists', () => {
    const custom: Partial<PillarCadence> = {
      bodycomp: { intervalDays: 30, priority: 'medium', reason: 'coach override' },
    };
    expect(getEffectiveInterval(recommended, custom, 'bodycomp')).toBe(30);
  });
...
describe('getEffectiveReason', () => {
  it('returns custom reason over recommended', () => {
    const recommended: PillarCadence = {
      bodycomp: { intervalDays: 14, priority: 'high', reason: 'auto reason' },
    ...
    const custom: Partial<PillarCadence> = {
      bodycomp: { intervalDays: 30, priority: 'medium', reason: 'coach said so' },
    };
    expect(getEffectiveReason(recommended, custom, 'bodycomp')).toBe('coach said so');
  });
...
describe('getEffectivePriority', () => {
  it('returns custom priority over recommended', () => {
    ...
    const custom: Partial<PillarCadence> = {
      bodycomp: { intervalDays: 30, priority: 'low', reason: 'downgraded' },
    };
    expect(getEffectivePriority(recommended, custom, 'bodycomp')).toBe('low');
  });
