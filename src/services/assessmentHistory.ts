...
export async function updatePostureAnalysis(
  ...
  const currentData = currentSnap.data() as { formData?: FormData; organizationId?: string };
  
  // Security check
  if (organizationId && currentData.organizationId && currentData.organizationId !== organizationId) {
