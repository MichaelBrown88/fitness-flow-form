      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
            <p className="text-xs text-slate-400 mb-1">Pipeline (Trial)</p>
          ...
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
            <p className="text-xs text-slate-400 mb-1">Churned (total)</p>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 sm:flex-initial min-w-[140px]">
            ...
            <Input
              ...
              className="pl-8 h-9 bg-slate-800 border-slate-700 text-white w-full sm:w-44"
            />
          </div>
          <Select value={filterStatus || 'all'} onValueChange={(v) => setFilterStatus(v === 'all' ? '' : v)}>
            <SelectTrigger className="h-9 w-28 bg-slate-800 border-slate-700 text-white">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-xs text-slate-400 whitespace-nowrap">Incomplete only</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <Checkbox checked={filterTest} onCheckedChange={(c) => setFilterTest(!!c)} />
            <span className="text-xs text-slate-400 whitespace-nowrap">Test only</span>
          </label>
        </div>
        <span className="text-xs text-slate-500">{filteredOrganizations.length} shown</span>
      </div>
        ...
        <>
          <div className="px-5 py-3 border-b border-slate-800 grid grid-cols-12 gap-4 text-xs text-slate-500 font-medium">
            <button
              ...
              className="col-span-2 text-left hover:text-slate-400 flex items-center gap-1"
            >
              Organization
              ...
              return (
                <div key={org.id} className="px-5 py-4 hover:bg-slate-800/30 transition-colors">
                  <div className="grid grid-cols-12 gap-4 items-center">
                    ...
                    <div className="col-span-1">
                      <span className="text-xs text-slate-400">{org.seatBlock != null ? `${org.seatBlock} clients` : (org.plan ?? '—')}</span>
                    </div>
                    <div className="col-span-1">
                      {org.isComped ? (
                    ...
                    <div className="col-span-1">
                      ...
                      {org.assessmentsThisMonth != null && org.assessmentsThisMonth > 0 && (
                        <span className="text-xs text-slate-500 block">MTD: {org.assessmentsThisMonth}</span>
                      )}
                    </div>
                    <div className="col-span-1">
          ...
          {hasMoreOrganizations && (
            <div className="px-5 py-4 flex justify-center border-t border-slate-800">
              <Button variant="outline" size="sm" onClick={loadMoreOrganizations} className="border-slate-700 text-slate-300 hover:bg-slate-800">
                Load more
              </Button>
            </div>
          )}
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-300 capitalize">{org.type.replace('_', ' ')}</p>
        </div>
        <div>
        ...
        <div>
          ...
          <p className="text-sm text-slate-300 capitalize">{org.plan}</p>
          {(org.seatBlock ?? org.clientSeats) != null && (org.seatBlock ?? org.clientSeats)! > 0 && (
            <p className="text-xs text-slate-500 mt-1">{org.seatBlock ?? org.clientSeats} clients (plan)</p>
          )}
        </div>
        <div>
        ...
        <div>
          ...
          <p className="text-sm text-slate-300 capitalize">{org.status}</p>
        </div>
        <div>
        ...
        <div>
          ...
          <p className="text-sm text-slate-300">{org.coachCount}</p>
        </div>
        <div>
          <p className="text-xs text-slate-500 mb-1">Clients</p>
          <p className="text-sm text-slate-300">{org.clientCount}</p>
        </div>
        <div>
          <p className="text-xs text-slate-500 mb-1">Assessments</p>
          <p className="text-sm text-slate-300">{org.assessmentCount}</p>
        </div>
        <div>
      ...
      <div className="flex gap-2 pt-4 border-t border-slate-800">
        <Button size="sm" variant="outline" className="text-xs border-slate-700 text-slate-300 hover:bg-slate-700" onClick={() => navigateToOrg(org.id)}>
