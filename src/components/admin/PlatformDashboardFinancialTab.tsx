    ...
    <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {revenueByRegion.byRegion.GB && (
              <div className="p-4 bg-slate-800/50 border border-slate-700 rounded-xl">
                <p className="text-xs text-slate-400 mb-1">UK</p>
            ...
            {revenueByRegion.byRegion.US && (
              <div className="p-4 bg-slate-800/50 border border-slate-700 rounded-xl">
                <p className="text-xs text-slate-400 mb-1">US</p>
            ...
            {revenueByRegion.byRegion.KW && (
              <div className="p-4 bg-slate-800/50 border border-slate-700 rounded-xl">
                <p className="text-xs text-slate-400 mb-1">Kuwait</p>
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="p-4 bg-slate-800/50 border border-slate-700 rounded-xl">
            <p className="text-xs text-slate-400 mb-1">Monthly Recurring Revenue (GBP)</p>
            <p className="text-2xl font-bold text-white">{formatCurrency(metrics?.mrrCents ?? 0, 'GBP')}</p>
          </div>
          <div className="p-4 bg-slate-800/50 border border-slate-700 rounded-xl">
            <p className="text-xs text-slate-400 mb-1">Annual Recurring Revenue (GBP)</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
          <p className="text-xs text-slate-400 mb-1">AI Costs (MTD)</p>
          <p className="text-2xl font-bold text-amber-400">{formatCurrency(metrics?.aiCostsMtdCents ?? 0, 'GBP')}</p>
        </div>
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
          <p className="text-xs text-slate-400 mb-1">AI Costs (All Time)</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
          ...
          {aiCostsByFeature.length === 0 ? (
            <p className="text-sm text-slate-500">No AI cost data this month</p>
          ) : (
            <div className="space-y-3">
              {aiCostsByFeature.map((item) => (
                <div key={item.feature} className="flex items-center justify-between p-3 bg-slate-800/30 rounded-lg">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-xs text-slate-500 ml-2">({formatNumber(item.count)} requests)</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
          ...
          {aiCostsByFeatureAllTime.length === 0 ? (
            <p className="text-sm text-slate-500">No AI cost data recorded</p>
          ) : (
            <div className="space-y-3">
