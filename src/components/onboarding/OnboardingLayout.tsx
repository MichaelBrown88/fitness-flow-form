...
export function OnboardingLayout({ currentStep, children, onBack, onBypass }: OnboardingLayoutProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-4 md:p-8">
      ...
      <GlassPanel className={`w-full max-w-4xl h-full sm:h-[90vh] md:h-[67.5vh] rounded-2xl sm:rounded-[2.5rem] relative overflow-hidden flex flex-col transition-all duration-500 ${isAnimating ? 'opacity-50 scale-[0.99]' : 'opacity-100 scale-100'}`}>
        ...
        {/* Header */}
        <div className="px-4 sm:px-6 md:px-8 py-4 sm:py-6 md:py-8 flex justify-between items-center z-20 border-b border-white/50">
          {currentStep >= 0 && onBack ? (
            <button 
              onClick={onBack} 
              className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white border border-slate-100 flex items-center justify-center hover:bg-slate-50 text-slate-500 transition-colors shadow-sm"
            >
          ...
          {currentStep >= 0 && currentStep < ONBOARDING_STEPS.length && (
            <div className="flex gap-1.5 sm:gap-2">
              {ONBOARDING_STEPS.map((_, index) => (
                <div 
                  key={index} 
                  className={`h-1 sm:h-1.5 rounded-full transition-all duration-700 ease-out ${
                    index === currentStep ? 'w-6 sm:w-8 bg-indigo-600' : 
                    index < currentStep ? 'w-1.5 sm:w-2 bg-indigo-200' : 'w-1.5 sm:w-2 bg-slate-200'
                  }`}
                />
              ))}
            </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            {user && (
              <button
                ...
                className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white border border-slate-100 flex items-center justify-center hover:bg-red-50 hover:border-red-200 text-slate-500 hover:text-red-600 transition-colors shadow-sm"
        ...
        {/* Content Scroll Area */}
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 md:px-12 py-4 sm:py-6 md:py-8 relative z-10">
          {children}
        </div>
      </GlassPanel>
    </div>
