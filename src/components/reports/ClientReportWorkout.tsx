...
export function ClientReportWorkout({ plan, goalLabel }: ClientReportWorkoutProps) {
        ...
        <div className="space-y-4">
          ...
          {plan.clientWorkout.warmUp.length > 0 && (
            <div className="bg-card rounded-xl border border-border p-5">
              <h3 className="text-sm font-bold text-foreground mb-3">Warm-up</h3>
              <div className="space-y-3">
                {plan.clientWorkout.warmUp.map((ex, i) => (
                  <div key={i} className="text-sm">
                    <div className="flex items-center justify-between">
                      <span className="text-foreground font-medium">{ex.name}</span>
                      <span className="text-muted-foreground text-xs">{ex.setsReps || ex.time || ''}</span>
                    </div>
                    {ex.addresses && <p className="text-xs text-muted-foreground italic mt-1">{ex.addresses}</p>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {plan.clientWorkout.exercises.length > 0 && (
            <div className="bg-card rounded-xl border-2 border-primary p-5">
              <h3 className="text-sm font-bold text-foreground mb-3">Main Workout</h3>
              <div className="space-y-3">
          ...
          {plan.clientWorkout.finisher && (
            <div className="bg-card rounded-xl border border-border p-5">
              <h3 className="text-sm font-bold text-foreground mb-3">Finisher</h3>
              <div className="text-sm">
                <div className="flex items-center justify-between">
                  <span className="text-foreground font-medium">{plan.clientWorkout.finisher.name}</span>
                  <span className="text-muted-foreground text-xs">
                    {plan.clientWorkout.finisher.setsReps || plan.clientWorkout.finisher.time || ''}
                  </span>
                </div>
                {plan.clientWorkout.finisher.addresses && (
                  <p className="text-xs text-muted-foreground italic mt-1">{plan.clientWorkout.finisher.addresses}</p>
                )}
              </div>
            </div>
          )}
      ...
      return (
        <div className="space-y-6">
          ...
          <div className="grid gap-6">
            ...
            <div className="bg-card rounded-3xl border border-border p-8 shadow-sm space-y-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-bold text-foreground">Performance Tuning (Warm-up)</h3>
                </div>
              ...
              <div className="grid gap-3">
                {plan.prioritizedExercises.groups
                  ...
                  .map((ex, i) => (
                    <div key={i} className="flex items-center justify-between p-3 rounded-xl border border-border bg-muted/50">
                      <div>
                      ...
                      <span className="text-xs font-medium text-foreground-tertiary">{ex.setsReps || '2 x 30s'}</span>
                    </div>
                  ))}
              </div>
            </div>
            ...
            {/* Main Block */}
            <div className="bg-card rounded-3xl border-2 border-primary p-8 shadow-md space-y-6 relative overflow-hidden">
              <div className="absolute top-0 right-0 -mt-2 -mr-2 bg-primary text-primary-foreground text-xs font-bold px-4 py-2 rotate-12 shadow-lg">
                GOAL BLOCK
              </div>
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-2xl bg-primary flex items-center justify-center text-primary-foreground font-bold italic">
                  B
                </div>
                <h3 className="text-lg font-bold text-foreground">Main Training Block (The Work)</h3>
              </div>
              <p className="text-sm text-muted-foreground">
                ...
                {plan.prioritizedExercises.goalExercises.slice(0, 3).map((goal: string, i: number) => (
                  <div key={i} className="flex items-center gap-3 p-4 rounded-2xl bg-brand-light/50 border border-primary/10">
                    <div className="h-8 w-8 rounded-full bg-primary flex items-center justify-center shrink-0">
                      <TargetIcon className="h-4 w-4 text-primary-foreground" />
                    </div>
                    <p className="text-sm font-bold text-foreground">{goal}</p>
                  </div>
                ))}
                {plan.prioritizedExercises.groups
                  .find((g) => g.priority === 'goal-focused')
                  ?.exercises.map((ex, i) => (
                    <div key={`ex-${i}`} className="flex items-center justify-between p-4 rounded-2xl border border-border bg-card">
                      <div>
                        <p className="text-sm font-bold text-foreground">{ex.name}</p>
                        <p className="text-xs text-muted-foreground italic mt-0.5">{ex.reason}</p>
                      </div>
                      <Badge className="bg-primary text-primary-foreground border-none text-xs font-bold">
  ...
  return (
    <section className="space-y-4">
      <h2 className="text-xl font-bold text-foreground">Your Sample Workout</h2>
      {workoutContent}
    </section>
  );
}
