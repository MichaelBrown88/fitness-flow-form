    ...
    <section className="w-full min-w-0 overflow-x-hidden">
      {!hideHeader && (
        <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 mb-2 sm:mb-3 md:mb-4 lg:mb-5 xl:mb-6">
          ...
          <h3 className="text-xs md:text-sm lg:text-base font-semibold text-foreground">Your Destination</h3>
        </div>
      )}
                  ...
                  <div className="p-4 sm:p-5 md:p-6 lg:p-8 xl:p-10">
                    <div className="flex flex-col md:flex-row gap-4 sm:gap-6 md:gap-8 lg:gap-10">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs sm:text-sm text-foreground-secondary leading-relaxed mb-4 sm:mb-6 md:mb-8">
                          {explanation}
                        </p>
                          ...
                          <ul className="space-y-2 sm:space-y-3 md:space-y-4">
                            {whatItEntails.map((item, j) => (
                              <li key={j} className="flex items-start gap-2 sm:gap-3">
                                ...
                                <span className="text-xs sm:text-sm text-foreground-secondary leading-relaxed">{item}</span>
                              </li>
                            ))}
                          </ul>
                        </div>
                      ...
                      {addressingItems.length > 0 && (
                        <div className="flex-1 border-t md:border-t-0 md:border-l border-border pt-6 sm:pt-8 md:pt-0 md:pl-4 lg:pl-6 xl:pl-10 min-w-0">
                          <p className="text-[10px] font-black text-foreground uppercase tracking-[0.15em] mb-3 sm:mb-4 md:mb-6">What We'll Address</p>
                          <ul className="space-y-3 sm:space-y-4 md:space-y-6">
                            {addressingItems.map((item, i) => (
                              <li key={i} className="flex items-start gap-2 sm:gap-3 md:gap-4 group">
                                <div className="p-1.5 sm:p-2 md:p-2.5 glass-label text-foreground-secondary rounded-lg shrink-0 group-hover:bg-card/80 group-hover:text-gradient-dark transition-apple">
                                  <item.icon className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5" />
                                </div>
                                <div className="min-w-0 flex-1">
                                  <span className="font-bold text-foreground text-xs md:text-sm block mb-0.5 sm:mb-1">{item.title}</span>
                                  <span className="text-xs md:text-sm text-muted-foreground block leading-relaxed">{item.desc}</span>
                                </div>
                              </li>
                            ))}
                          </ul>
