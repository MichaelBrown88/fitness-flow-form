...
export const LandingCapabilityRow = forwardRef<HTMLDivElement, LandingCapabilityRowProps>(
  function LandingCapabilityRow({ row }, ref) {
    ...
    return (
      <div
        ref={ref}
        className="grid items-center gap-10 rounded-lg border border-border/90 bg-card/80 p-6 shadow-sm backdrop-blur-sm sm:p-8 lg:grid-cols-2 lg:gap-16 lg:p-10 dark:border-border dark:bg-card/40"
      >
        {row.imageSide === 'left' ? (
          <>
